...
if echo "$CHECK1" | grep -q HTTPServer; then
  ...
  rm -fv /etc/ld.so.conf.d/httpd-lib.conf /etc/ld.so.cache
  REBOOT_NEEDED=1
...
############################################
# FASE DI BACKUP (UTENTE root)
############################################
info "INIZIO FASE DI BACKUP"

mkdir -pv \
  "$BACKUP_ROOT/IHS/logs" \
  "$BACKUP_ROOT/IHS/htdocs" \
  "$BACKUP_ROOT/WebSphere/Plugins/config"

# Backup directory custom IHS
find "$HTTPD_ROOT" -mindepth 1 -maxdepth 1 -type d \
...
# Backup htdocs (escluso images)
find "$HTTPD_ROOT/htdocs" -mindepth 1 -maxdepth 1 -type d \
! -name images -exec cp -a {} "$BACKUP_ROOT/IHS/htdocs" \;

# Backup Plugins config
find /prod/IBM/WebSphere/Plugins/config -mindepth 1 -maxdepth 1 -type d \
...
"$IM_INSTALLATION_HOME"/eclipse/tools/imcl uninstall com.ibm.websphere.PLG.v85

[ $? -eq 0 ] || error_exit "Errore disinstallazione Plugins 8.5"
rm -rf /prod/IBM/WebSphere/Plugins
...
"$IM_INSTALLATION_HOME"/eclipse/tools/imcl uninstall com.ibm.websphere.IHS.v85

[ $? -eq 0 ] || error_exit "Errore disinstallazione IHS 8.5"
rm -rf "$HTTPD_ROOT"
...
mkdir -pv /prod/IBM/IM_INSTALL
cd /prod/IBM/IM_INSTALL

curl -o agent.installer.linux.zip \
http://stcwebiblprepo1.srv.sogei.it/IM/agent.installer.linux.gtk.x86_64_1.10.1003.20250827_1041.zip

unzip -q agent.installer.linux.zip -d im
im/installc -acceptLicense -showProgress \
...
"$IM_INSTALLATION_HOME"/eclipse/tools/imcl install \
...
cp -a "$BACKUP_ROOT/IHS/"* "$HTTPD_ROOT/"
cp -a "$BACKUP_ROOT/WebSphere/Plugins/"* /prod/IBM/WebSphere/Plugins/

############################################
# OWNER
############################################
chown -R webihs:webihs "$HTTPD_ROOT" /prod/IBM/WebSphere/Plugins
